    ...
    private void readContacts(Context context) {
        ...
        if(cursor.getCount() > 0) {
            while(cursor.moveToNext()) {
                ...
                String lastTimeContacted = cursor.getString( cursor.getColumnIndex( ContactsContract.Contacts.LAST_TIME_CONTACTED));
                contact.setLastTimeContacted(lastTimeContacted);

                String contactStatus = cursor.getString( cursor.getColumnIndex( ContactsContract.Contacts.CONTACT_STATUS));
                ...
                map.put("lastTimeContacted", "lastTimeContacted: " +  contact.getLastTimeContacted());
                map.put("contactStatus", "contactStatus: " +  contact.getContactStatus());
                contactList.add(map);
            }
        }
                }
